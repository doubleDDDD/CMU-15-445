...
public:
    ...
    inline int32_t GetOffset(const int column_id) const { return columns[column_id].GetOffset(); }

    inline TypeId GetType(const int column_id) const { return columns[column_id].GetType(); }
    ...
    inline int32_t GetLength(const int column_id) const { return columns[column_id].GetFixedLength(); }

    inline int32_t GetVariableLength(const int column_id) const { return columns[column_id].GetVariableLength(); }

    inline bool IsInlined(const int column_id) const { return columns[column_id].IsInlined(); }

    inline const Column GetColumn(const int column_id) const { return columns[column_id]; }

    // column id start with 0
    inline int GetColumnID(std::string col_name) const {
        int i;
        for (i = 0; i < GetColumnCount(); ++i) { if (columns[i].GetName() == col_name) { return i; } }
        return -1;
    }

    inline const std::vector<int> &GetUnlinedColumns() const { return uninlined_columns; }
    ...
    inline int GetUnlinedColumnCount() const { return static_cast<int>(uninlined_columns.size()); }
