...
public:
    ...
    Column(TypeId value_type, int32_t column_length, std::string column_name)
        ...
        if (is_inlined && column_length <= 0) { throw Exception(EXCEPTION_TYPE_CONSTRAINT, "inline type must pass in column_length"); }
        SetLength(column_length);  // column_length 就是其所占字节数
    }
...
private:
    // only varchar is not inline, the length is specified by user, like var(13)
    // 其它数据类型的长度都是固定的，即内建的数据类型

    // value type of column
    TypeId column_type; //  = TypeId::INVALID; or BIGINT or VARCHAR
