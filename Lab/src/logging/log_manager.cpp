 ...
 * 
 * 这个与 ext4 的 jbd2 也同样是一个单独的 thread
 */
    ...
    {
        {
            ...
            while (log_buffer_size_ == 0) {
                ...
                if (ret == std::cv_status::no_timeout || flush_thread_on == false) { break; }
            }
            SwapBuffer();
        }

        // 写入磁盘，这里也仅仅是 flush 到内核
        disk_manager_->WriteLog(flush_buffer_, flush_buffer_size_);
        std::unique_lock<std::mutex> lock(latch_);

        auto lsn = lastLsn(flush_buffer_, flush_buffer_size_);
        if (lsn != INVALID_LSN) { SetPersistentLSN(lsn); }
...
void LogManager::StopFlushThread() 
{
    ...
    if (flush_thread_on == true) 
    {
        ...
        lock.unlock();
        cv_.notify_all();  // 通知所有在 cv 上等待的线程
...
void LogManager::WaitUntilBgTaskFinish() 
{
    ...
    while (flush_buffer_size_ != 0) { flushed.wait(condWait); }
}
...
lsn_t LogManager::AppendLogRecord(LogRecord &log_record) 
{
    ...
    log_record.lsn_ = next_lsn_++;   // 这个地方是要原子分配的

    if (size + log_buffer_size_ > LOG_BUFFER_SIZE) {
        // 叫醒后台线程
        GetBgTaskToWork();  // 启动后台线程
        guard2.unlock();
        WaitUntilBgTaskFinish();  // 反正先把数据全部 flush 到 kernel 先
